...
const FEATURES = [
  ...
  },
  {
    ...
    desc: "Capture credentials, session cookies, and OAuth refresh tokens through BITB (Browser-in-the-Browser) and iframe-based flows. Supports Microsoft 365, Google, and custom OAuth flows, with Telegram notifications on capture.",
  },
  {
    icon: Shield,
    title: "Anti-Detection & WAF Bypass",
    desc: "Browser-grade TLS ciphers, fingerprint randomization, user-agent rotation, bot detection and telemetry stubbing, geo-spoofing, and Cloudflare WARP egress — so proxied traffic blends in with real browser traffic.",
  },
  {
    icon: Cpu,
    title: "Rule Engine & Analytics",
    desc: "CSS-selector-based rules track elements and trigger actions on target pages. Every request streams into real-time analytics: live visitor feed, unique visitors, bot detection, and per-site stats.",
  },
  {
    icon: Server,
    title: "Self-Hosted Infrastructure",
    desc: "Ships as Dockerized services: Node.js engine, nginx, and Firestore-backed persistence. Everything is managed from the dashboard — domains, certificates, secrets, WARP, and site configuration.",
  },
];
...
const FLOW = [
  ...
  { step: "2", title: "Create a site", desc: "Point a subdomain at any target URL, as a proxy site or a capture site (BITB / redirect / OAuth consent)." },
  { step: "3", title: "Configure modules", desc: "Attach injection templates, capture modules, and CSS-selector rules to shape the proxied experience." },
  { step: "4", title: "Go live & monitor", desc: "Switch the site to live mode, then watch visits, captures, and loot arrive in real time." },
];
      ...
      <div className="mx-auto w-full max-w-[760px] px-[16px] pb-[100px] pt-[14px] md:px-[24px]">
        ...
        <div className="flex items-center gap-[14px]">
          ...
          <div>
            ...
            <p className="mt-[1px] text-[13px] text-fg3">
              Self-hosted reverse proxy &amp; session capture platform — v3.0
            </p>
          </div>
        </div>
        ...
        <div className="mt-[24px]">
          <p className="text-[14px] leading-[22px] text-fg2">
            ...
            traffic: rewrite content, inject code, capture sessions, and stream every request into
            real-time analytics. Everything runs on your own infrastructure — no third-party relay.
          </p>
          <p className="mt-[10px] text-[13px] leading-[20px] text-fg3">
            It is built for a single operator: one dashboard manages domains, sites, modules,
            payloads, captured loot, egress (including WARP), and domain reputation checks.
          </p>
        </div>
        ...
        <div className="mt-[36px]">
          ...
          <div className="mt-[14px] grid grid-cols-1 gap-[12px] md:grid-cols-2">
            {FEATURES.map((f) => (
              <div key={f.title} className="rounded-[8px] border border-line bg-surface p-[16px]">
                ...
                <p className="mt-[10px] text-[12.5px] leading-[19px] text-fg3">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-[36px]">
          ...
          <div className="mt-[12px] flex flex-wrap gap-[8px]">
            {[
              ...
              "Tailwind CSS", "certbot", "WARP egress",
            ].map((tech) => (
              <span
                key={tech}
        ...
        {}
        <div className="mt-[36px] flex items-start gap-[12px] rounded-[8px] border border-errorBorder bg-errorSurface p-[18px]">
          <AlertTriangle className="mt-[1px] h-[18px] w-[18px] shrink-0 text-accent" />
          <div>
            ...
            <p className="mt-[6px] text-[12.5px] leading-[19px] text-fg3">
              ...
              before proxying, intercepting, or capturing data from any website or user. Unauthorized
              use of this tool to capture credentials, session tokens, or personal data without
              explicit consent may violate computer fraud, privacy, and wiretapping laws in your
