...
import { ChevronDown, Trash2, CheckCircle, AlertCircle, AlertTriangle, Info, X, ArrowUpRight } from "lucide-react";
...
export function BlueButton({ children, className = "", ...props }) {
  return (
    <button
      ...
      {...props}
    >
      <span className="border-b-2 border-accent pb-[2px] transition-colors group-hover:border-accentHover">
        {children}
      </span>
    </button>
  );
    ...
    return (
      <div className={cx("relative flex items-center justify-center overflow-hidden bg-panel", className)}>
        <div
          ...
          style={{
            backgroundImage: "radial-gradient(circle, var(--dotted-underline) 1px, transparent 1px)",
...
export function SiteCard({ site, healthStatus, ruleCount, siteUrl, onToggleLive, onDelete }) {
      ...
      <div className="p-[20px]">
        ...
        <div className="mt-[18px] flex items-center justify-end gap-[14px]">
          ...
          <button
            ...
            className={cx(
              "group inline-flex h-[28px] items-center text-[12.5px] font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-40",
              danger
                ? "text-error"
                : warning
                  ? "text-warning"
