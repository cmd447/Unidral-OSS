...
async function request(method, path, body, retries = 2) {
  ...
  if (!response) {
    throw new Error(lastErr && lastErr.name === "AbortError" ? "Request timed out." : "Cannot reach the Unidral engine. Check your connection.");
  }
...
export const api = {
  ...
  createTemplate: payload => request("POST", "/engine/templates", payload),
  updateTemplate: (id, payload) => request("PUT", `/engine/templates/${id}`, payload),
  deleteTemplate: id => request("DELETE", `/engine/templates/${id}`),
