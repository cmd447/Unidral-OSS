...
if command -v curl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
  API_URL="http://unidral-server:4000/api"
  ...
  if [ -z "${DOMAINS_JSON}" ]; then
    echo "[nginx] Docker network fetch failed, trying 127.0.0.1:4000..."
    DOMAINS_JSON=$(curl -sf --max-time 5 "http://127.0.0.1:4000/api/domains" 2>/dev/null) || true
  fi
